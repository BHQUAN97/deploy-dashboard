...
export function DeployDrawer({ project, runId, open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      ...
      if (ev.type === 'error') {
        setError(ev.message ?? 'Could not load run status')
        es.close()
      }
    ...
    es.onerror = () => {
      setError('Connection lost while streaming GitHub Actions status')
      es.close()
    }
    return () => {
    ...
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent className="bg-zinc-900 border-zinc-700 text-zinc-100 w-[480px] sm:max-w-[480px] flex flex-col p-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            {!done && !runId && (
              <div className="flex items-center gap-2 text-xs text-amber-300">
                ...
                Workflow triggered. Waiting for GitHub run id...
              </div>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {error && (
            <div className="mb-4 rounded border border-red-800 bg-red-950/30 px-3 py-2 text-sm text-red-300">
              {error}
            </div>
          )}
          {jobs.length === 0 && !done && (
            <div className="flex items-center justify-center h-32 text-zinc-600 text-sm">
              ...
              {runId ? 'Waiting for jobs...' : 'GitHub accepted the workflow, but the run is not visible yet.'}
            </div>
          )}
          <JobSteps jobs={jobs} />
        </div>
